...
Pathotype.Data <- read_csv(here("Prctice data set.csv"), na = "")
```
...
Do not change Pathotype.Data in any of the functions. You need only change the three spaces after Pathotype.Data to your associated column headings.

###Instructions

"Isolate" should be renamed to the column header for the column which identifies the isolates tested
"Percent.Susceptible" should be renamed to the column header for the column which identifies the percent susceptible plants for each gene
...
The value in "Distribution_of_Susceptibilities(Pathotype.Data, "Isolate:, "Percent.Susceptible", "Gene", 60)" (in this case, 60), sets the cutoff for susceptible reactions. For example, currently all genes with 60% or more of the plants rated susceptible will return a "1" in The following scripts. You can change this to whatever percentage you require for your study.
...
```{r Running function}
Suceptibilities <- Distribution_of_Susceptibilities(Pathotype.Data, "Isolate", "percent.Susceptible", "Gene", 60)
```
...
You will need the change "Isolate", "Percent.Susceptible", and "Gene" again in this function to your correct column headers in your dataset.
Again, you can change your susceptible cutoff value here for your dataset
```{r Run function}
complexities <- Distribution_of_Complexities(Pathotype.Data, "Isolate", "percent.Susceptible", "Gene", 60)
```
...
Again, you can change your susceptible cutoff value here for your dataset.
```{r Pathotype Freq}
path.freq <- Pathotype.frequency.dist(Pathotype.Data, "Isolate", "percent.Susceptible", "Gene", 60) 
```
...
```{r echo = FALSE}
path.freq$pathotypes_distribution
```
...
```{r}
diversity <- Diversity_index(Pathotype.Data, "Isolate", "percent.Susceptible", "Gene", 60)
```
