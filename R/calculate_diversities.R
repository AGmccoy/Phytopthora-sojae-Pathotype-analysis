...
#'
#' Custom [print()] method for `hagis.diversities` objects. Index values are
#' rounded to two digits for printing.
#'
...
print.hagis.diversities <- function(x,
  ...
  cat("\n")
  cat("Indices\n")
  cat("Simple  ", round(x[[5]], 2), "\n")
  cat("Gleason ", round(x[[6]], 2), "\n")
  cat("Shannon ", round(x[[7]], 2), "\n")
  cat("Simpson ", round(x[[8]], 2), "\n")
  cat("Evenness ", round(x[[9]], 2), "\n")
  cat("\n")
  invisible(x)
}
...
#'
#' Prints a hagis diversities in Pandoc's markdown. Index values are rounded to
#' two digits for printing.
...
pander.hagis.diversities <-
  function(x, caption = attr(x, "caption"), ...) {
    pander::pander(
      data.frame(
        "Simple" = round(x$Simple, 2),
        "Gleason" = round(x$Gleason, 2),
        "Shannon" = round(x$Shannon, 2),
        "Simpson" = round(x$Simpson, 2),
        "Evenness" = round(x$Evenness, 2),
      ),
      caption = paste0(
